...
Native Windows 11 support is an adopted maintenance target, not a released capability. Its fixed profile is Windows Terminal plus PowerShell 7 with an independent Windows path/ACL/PowerShell composition; see [Native Windows 11 Maintenance](docs/windows-maintenance.md). Do not interpret the presence of Pi's PowerShell tool or that transition document as an existing Windows Access Gate guarantee.
