...
> **HARD GATE** — Requires git context (branch with merge-base or diff). Run the scan in a Herdr Agent and return the structured report to the main session. The main session records it in the current security-review section of the active Task Record (`docs/task.md` or `docs/task-<topic>.md`) and creates a `Kind: maintenance` Task Record in `docs/task.md` (Kind per principles.md Project Records — Record Lifecycle) when no active task exists.
