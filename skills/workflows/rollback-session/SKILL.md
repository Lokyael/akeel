...
Use `git restore` or `git revert`. Destructive recovery commands (`git reset --hard`, `git checkout --`, `git clean`) run only with explicit user intent (per principles.md §10); confirm the exact paths and scope before running them.
