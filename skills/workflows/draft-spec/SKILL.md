...
If the source is an `F-xxx` entry, proceed only after the user explicitly commits to the work in the current conversation. Add optional `Origin: F-xxx` metadata to the Task Record and remove the Future Record in the same change. The existence, wording, `Review On`, or `Trigger` of a Future Record is never sufficient approval.
