...
- **Status**: `draft`. After design review, transition to `in-progress`, then `verified`. Per principles.md Project Records — Record Lifecycle.
...
- **Out of scope**: Per principles.md §8 format. Skip if nothing is excluded.
...
If the source is a `C-xxx` entry, proceed only after the user explicitly commits to the work in the current conversation (migration per principles.md Project Records — Migration Protocol). Add optional `Origin: C-xxx` metadata to the Task Record and remove the Candidate Record in the same change. The existence, wording, or `Revisit condition` of a Candidate Record is never sufficient approval.
