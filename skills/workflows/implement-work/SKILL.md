...
**Lifecycle:** Require a Task Record in `docs/task.md` or `docs/task-<topic>.md`; if none exists, ask: "Where is the Task Record, or what task should be recorded?" Set it from `draft` to `in-progress`, then confirm reachable Git history contains its heading, approved Requirements, and necessary Design/Plan. Otherwise commit only the Task Record as a checkpoint — no implementation or unrelated changes. If repository state or commit authority prevents that commit, stop and ask the user. A slot advance or commit-message mention is not a checkpoint.

Use approved Plan Slices as the implementation order and Requirements-to-verification map. A Task that fits one coherent test cycle can proceed directly; multi-step work receives an implementation-ready Plan through `implementation-planning` before code changes. Commit Task updates only for cross-session continuity, handoff, or material authority changes, never step logs.
...
Re-read every Task Requirement and verify it with concrete evidence; passing tests alone does not prove completion. After the Task Owner disposes all findings from the unchanged READY surface, confirm the checkpoint remains reachable, freeze the accepted Requirements in the review packet, apply durable updates, set the Task to `verified`, and clear it (per principles.md Next-ID slots). Repeat steps 1–5 against that final surface using the frozen Requirements for preflight and Requirements review; closure does not recreate the Task. Commit the clearing change only while that surface remains unchanged and all blocking findings are resolved. Never squash or rewrite away the only checkpoint.
