...
defineAdapterTests("read", [
  ...
  { cmd: "cat - file.txt", name: "cat - skips stdin and keeps files", cls: "inspect", intents: [{ operation: "read", rawPath: "file.txt" }], opaque: false },
  { cmd: "cat -", name: "bare dash is stdin, not opaque", cls: "inspect", intents: [], opaque: false },
  { cmd: "head -15 file.txt", name: "numeric short count is consumed, not opaque", cls: "inspect", intents: [{ operation: "read", rawPath: "file.txt" }], opaque: false },
