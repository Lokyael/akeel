...
function agentDir(): { readonly path: string; readonly cleanup: () => void } {
  const path = mkdtempSync(join(tmpdir(), "pi-keel-policy-"));
  mkdirSync(join(path, "pi-keel"), { recursive: true });
...
test("loads and freezes the new global policy.yaml schema", () => {
  ...
  try {
    writeFileSync(join(agent.path, "pi-keel", "policy.yaml"), [
      "paths:",
      "  read: allow",
      "  write: ask",
      "commands:",
...
test("rejects malformed YAML and legacy configuration fields", () => {
  const agent = agentDir();
  try {
    writeFileSync(join(agent.path, "pi-keel", "policy.yaml"), "paths: [");
    assert.deepEqual(loadPolicyFile(agent.path), { kind: "error" });

    writeFileSync(join(agent.path, "pi-keel", "policy.yaml"), "defaultProfile: keel-develop\n");
    assert.deepEqual(loadPolicyFile(agent.path), { kind: "error" });
  } finally {
    agent.cleanup();
  }
