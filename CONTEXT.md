...
- **系统分发与组件拓扑（Distribution & Packaging，D-086, D-090, D-098）**：AKeel 的用户能力仍是三个可独立安装的 package：`akeel-guidance`、`akeel-access-gate` 和 `akeel-context-pruner`；根 `akeel` package 提供全量 manifest。无 Pi resources 的 `akeel-platform-runtime` 仅向 Guidance 与 Access Gate 提供同源的平台证据与 runtime primitives，不是共享单例，也不拥有 Policy、GateSession、Artifact Exchange lifecycle 或 Prompt Surface；Context Pruner 保持独立。
- **提示词面与工程纪律注入（Prompt Surface & Guidance，D-030, D-053, D-073, D-084）**：`packages/guidance/src/bootstrap/` 通过 `before_agent_start` 注入工程原则；Project Records 遵循宿主对称性。Context Pruner 独立维护封闭的 Bash/PowerShell 正向测试 recognizer，只有命令与运行器成功证据完整时才投影模型视图；原始会话和用户 `bashExecution` 保持无损。Skills 按 `disciplines/` 与 `workflows/` 分责；Policy 数据不进入 LLM 上下文。
- **Access Gate 单一授权信任链（Single Trust Chain Pipeline，D-059, D-060, D-087, D-098）**：受管 Direct 与活动 profile Shell 统一进入 Canonical → Admission → Mandatory Boundary → Policy → Verdict；平台发行 sealed Path Proof 与 Root Catalog relations，公共核心不解释 pathname。授权链一致，执行机制按宿主事实实现：Direct 均由 Pi 原生执行，Linux Bash 为 Pi passthrough，Windows PowerShell 使用 AKeel-owned executor 与单次绑定。当前 Windows profile 仍在迁移期，不构成发布支持。
...
- [D-098 Unified first-class platform profiles and evidence authority](docs/decisions.md#d-098-unified-first-class-platform-profiles-and-evidence-authority)
