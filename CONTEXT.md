...
- **Platform Profile**：AKeel 对一个原生宿主组合的封闭支持合同；Linux profile 使用 Bash/Linux 路径与 mode 证据，目标 Windows profile 使用 Windows 11、Windows Terminal、PowerShell Core `>=7.4 <8`、本地 NTFS 路径与 SID/DACL/reparse 证据，并以同一现代 UTF-8 no BOM、`Standard` native argument-passing 配置支持英语与简体中文环境。平台在 extension runtime 加载时选择一次 composition，运行时资产、Policy Snapshot、path proof 与授权证据不跨 profile 混用。
- **Platform Runtime Authority**：无 Pi extension surface 的第一方内部支持能力，向 Guidance 与 Access Gate 发行平台标记的 path/private-filesystem/process/runtime-root 证据与 primitive；Linux adapter 保留 pathname/UID/mode 合同，目标 Windows adapter 通过受限 host service 取得 Win32 handle、file ID、reparse、SID/DACL 和 process identity。它不拥有 Policy、session/run lifecycle 或清理权。
- **Platform Path Proof**：活动 profile 对一个 native path 只解析一次后发行的 sealed 证据，包含 platform domain、literal/display、canonical location、object/traversal facts、risk flags 以及与 session Root Catalog 的关系；公共授权核心不解析其 native string，也不跨 profile 比较。
- **Windows Evidence Host**：目标 Windows profile 在 session 内用经验证的同一个 `pwsh.exe` 启动的 no-profile/non-interactive 私有协议进程；它承载官方 PowerShell AST parser 和 AKeel managed Win32 evidence assembly，只解析语法和查询 OS evidence，不执行模型命令。
...
- **Session Resource Envelope**：Access Gate 为一个 Pi session 在活动 profile 的 `sessions/session-*/` role root 创建的受管临时资源，包含 metadata、PID/creation identity lock 与唯一 stagingRoot；当前 Linux 物理根是 `/tmp/akeel`，目标 Windows 根由 local-NTFS user runtime environment 发行。正常 shutdown 精确删除，异常 residue 进入 D-088 retention。
- **Workflow Run**：同一 Task Owner 拥有的一次 bounded 临时 workflow attempt，位于活动 profile 的 `runs/run-*/` role root；当前 Linux 物理根是 `/tmp/akeel`，消费者使用 Artifact Exchange 返回的 exact path 而不拼接平台路径。它不是 Task Record，可包含 packet、child artifacts、control receipts、quarantine 与 transport diagnostics；每个 Owner session 最多保留 8 次 reservation。
...
- **系统分发与组件拓扑（Distribution & Packaging，D-086, D-090）**：AKeel 的用户能力仍是三个可独立安装的 package：`akeel-guidance`（bootstrap、skills、Artifact Exchange、Handoff Store 与 Record Containers 校验工具）、`akeel-access-gate`（Access Gate 准入引擎）和 `akeel-context-pruner`（测试输出上下文裁剪）；根 `akeel` package 提供三者的全量 manifest。已采纳的内部 `akeel-platform-runtime` support package 不声明 Pi resources，只集中 Guidance/Access Gate 共用的平台证据与 Windows host assets。外部通过 Pi 标准接口集成；各包分发目录归入能力资产域实施防篡改硬拦截，工作区源码 checkout 是唯一合法修改源。
- **提示词面与工程纪律注入（Prompt Surface & Guidance，D-030, D-053, D-073, D-084）**：`packages/guidance/src/bootstrap/` 通过 `before_agent_start` 原生注入包含工程原则的系统提示词 sections，天然具备会话转录持久化与 compaction 继承能力，Project Records 规范遵循宿主对称性（Host Symmetry Invariant），自仓与用户项目平等消费 guidance 提供的确定性容器校验工具；`packages/context-pruner/src/context-pruner/` 在构建模型 context 时无模型介入地将活动 profile positive recognizer 证明的独立测试命令与受支持运行器正向成功摘要投影为裁剪视图，Bash 与目标 PowerShell grammar 分离，用户终端执行（`bashExecution`）与原始会话无损保留，TUI 呈现受限于临时 Human-only 视图。Skills 严格按作者职责划分为 `disciplines/` 与 `workflows/`，按需全量加载；Policy Snapshot、配置文件与活动策略状态严格对 LLM 隔离，模型仅在失败路径接触静态有界 Guidance。
- **Access Gate 单一授权信任链（Single Trust Chain Pipeline，D-059, D-060, D-087）**：模型发起的受管 Direct 与活动 Platform Profile 的受管 Shell 统一进入单向信任链：Canonical facade 封装私有语义车道，可异步取得 native evidence，对请求执行一次权威、有界解释并发行 opaque `CanonicalCompilation`；下游不得从原始请求重建同一事实。路径事实是 platform-tagged sealed Path Proof，其 Root Catalog relations 供 Admission、Mandatory Boundary 与 Policy 纯投影消费，公共核心不解释 pathname。授权依次经过不可放宽 Mandatory Boundary 与 Configured Policy，Pi 宿主独占审批 UI；Windows 目标另外以单次 execution ticket 绑定获准输入与 AKeel-owned PowerShell executor。当前生产 composition 仅实现 Linux/Bash；Windows 目标在 T-0168 完成验收前不构成运行时支持。
...
- **Shell 分析器注册表与有界执行流（Shell Analyzers & Execution Flows，D-067, D-087, D-091）**：每个 Platform Profile 拥有封闭 Shell 编译车道，已知程序族可把平台已证明的选项、路径和 effect 投影到共享 Policy Kernel；未建模语法、动态执行、无界修改与外部/网络 transport 严格 fail-closed。当前 Linux/Bash 车道覆盖有界 Git、基础工具、构建工具与 `/dev/null`/静态管道；目标 Windows 车道从 PowerShell Core `>=7.4 <8` 的 official AST 投影独立建立 v1 single-command literal subset，cmdlet/provider/pipeline/redirection/script/dynamic/process forms 未证明前均拒绝，不复用 Bash parser 或路径事实。
...
- **策略预设与会话暂存区生命周期（Policy Presets & Session Lifecycle，D-069, D-088, D-097）**：生产入口仅读取全局 `policy.yaml`，内置 `review`、`guided`、`develop` 并支持自定义 preset；活动 profile 把全部 preset roots 编译进 bounded Root Catalog，Snapshot 只消费同域 root IDs，系统硬边界始终优先。`/policy` 提供临时选择面板、动态切换与状态查询；支持显式关闭 Access Gate。每个会话在 profile-owned runtime root 下拥有专属 Session Resource Envelope（含 metadata、PID/creation identity lock 与唯一 stagingRoot）；Linux 以 owner/mode、Windows 目标以 owner/SID/protected-DACL/reparse evidence 控制，正常退出精确清理，异常残留按 retention 配额回收。
...
- 当前 Linux profile 不拦截用户直接 Shell 命令、`shellCommandPrefix`、Shell `spawnHook`、tool override、custom tool backend、未知 Direct tool surface 或其他 Extension 的直接操作。活动 Platform Profile 以外的模型 Shell 是显式 unsupported surface：Linux composition 阻断模型 `powershell`，目标 Windows composition 阻断模型 `bash`。Windows 发布合同只覆盖 AKeel-owned PowerShell replacement 和验收过的 Pi Direct sources；第三方同名 tool/后端或 profile 外入口使 Windows composition fail-closed 或落在支持范围外。PowerShell 启动的 external executable/helper 仍按已知或 opaque 调用链治理；用户/RPC/SDK Bash 不因 Windows profile 获得保证。
...
- 当前 Linux Shell 只支持显式定义、可静态证明且资源有界的 Bash 子集；管道、`/dev/null`、已知程序、destroy、Git transport 和 opaque 的具体边界仍由现有 Linux 合同定义。目标 Windows Shell 只支持未来由 official AST 与独立 contract tests 证明的 PowerShell Core `>=7.4 <8` v1 子集；在 T-0168 完成前不存在可复用的 PowerShell 准入结论。任何 profile 的 `commands.opaque: allow` 都不提供运行期 sandbox、路径强制、network 或 descendant-process confinement。
