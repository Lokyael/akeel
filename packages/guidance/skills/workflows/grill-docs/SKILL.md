...
2. Fix the source snapshot with its base/head OIDs and relevant refs. If relevant state is uncommitted, include captured Git status, staged and unstaged diffs, untracked inventory/content, and a bounded diff artifact in the immutable Owner packet; a new worktree never inherits dirty state implicitly. A read-only child treats that packet as its Git authority and must not execute live Git or refresh repository state; missing or stale evidence returns to the Owner for a new packet.
