...
// - 嵌套 wrapper 由 parser 在 wrapper-args 状态下入栈，wrapper positional 已消费丢弃；
// - 本模块只做纯出栈：弹出 wrapper 链，不再做猜测、重构或递归（promotion /
//   guessExecutable / removeFromArgs / slice / MAX_UNWRAP_DEPTH 已删除，D-037）。
...
export function normalizeCommand(node: ShellCommandNode): NormalizedCommand {
  return {
    wrappers: [],
    // 弹出 wrapper 链：命令的真实参数由 parser 保证，无需再裁切
    command: node.wrapper.length > 0 ? { ...node, wrapper: [] } : node,
