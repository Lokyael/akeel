// access-gate/ui/footer-install.ts — Profile Footer 安装入口（/profile status 渲染见 profile-status.ts）
...
export function disposeProfileFooter(footer: ProfileFooterHandle | undefined): void {
  footer?.dispose();
}
