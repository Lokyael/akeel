...
Task `Kind` is `feature | bug | refactor | investigation | maintenance`. Keep `Out of Scope`, Requirements, Design, Plan, Evidence, and a durable-update checklist in one Task file. Before implementation or clearing, every T-ID must have a complete Task Record in reachable Git history containing approved Requirements and necessary Design/Plan; slot advancement or a commit-message mention does not count. For a new material Task, the minimum lifecycle is two commits: an implementation-before checkpoint and a later completion commit. Code commits may occur between them, but Task Record updates are event-driven — only cross-session continuity, handoff, or material authority changes justify one; never step logs.

After verification, confirm the checkpoint remains reachable, apply needed updates to `CONTEXT.md` and `docs/decisions.md`, and clear the Task in the completion commit before starting another Task. Never defer clearing to a later Task, or squash or rewrite away the only checkpoint. Git or an adopted external tracker retains process history; do not create a default archive. Update one record through an iterative design arc and clear it only when the work lands, rather than allocating an ID per iteration.
