...
- **引用验证**：`per principles.md Quick Reference — X` / `per principles.md Project Records — X` 锚点真实存在且承载所引语义；被删内容的语义存活于引用目标。
- **引用与内嵌取舍（引用可靠性，D-054）**：引用是共享规则的低频定位手段，不是技能默认形态——操作步骤与守卫留在动作点。指针化前按四问：① 目标在同一读取/注入面且短而高显著（同文档相邻、guidance 当下渲染）；跨文件引用解析时付一次真实读取，仅在单源收益超过读取成本时使用；长细节段接受方必须内嵌。② 执行必需或 do-not-X 守卫（审批否决、分类守卫、防误删）必须留在动作点内嵌。③ 解析失败可测或有情境兜底，否则静默失败。④ 删除量不足指针固定成本（措辞+解析）的短句不指针化。"既有引用存在"不证明映射可靠、常规或正确；本判据约束新改动，存量按同一标准再审计、不自动回退。
...
- **精简表达**：不写推论（文件固有性质不写成规则，如条目升序则末尾即最大）与派生数据（具体编号由规则推导，不手工维护）；删重复措辞、自明解释与冗余例子（每类保留 1–3 个代表例）。**必要强调除外——刻意的重复（铁律/门禁类重申、动作点复述、唯一载体落点句）是行为守卫不是冗余，判据是删除后行为是否可能偏离，而非行数。**
