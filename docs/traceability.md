...
| [Pi coding-agent extension guide](https://pi.dev/docs/latest/extensions)、[extension types](https://github.com/earendil-works/pi/blob/13cbf77df2396303013a41646bcfa77b4271ae56/packages/coding-agent/src/core/extensions/types.ts) 与 [package rules](https://pi.dev/docs/latest/packages) | Pi release `v0.86.1`, npm gitHead `13cbf77df2396303013a41646bcfa77b4271ae56` | conceptual reference / runtime development type source | 当前 AKeel 使用的官方 `ExtensionAPI`、`ExtensionContext`、`context`/`before_agent_start`/session replacement contracts，以及核心包由 Pi 宿主提供、package 使用 `peerDependencies: "*"` 的发布合同；当前类型由根开发依赖跟随 npm `latest` 获取，AKeel 不复制 Pi 内部实现 | Pi 仓库 MIT；本仓库未复制其代码 |
