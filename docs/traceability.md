...
| [Pi coding-agent extension guide](https://github.com/earendil-works/pi/blob/v0.87.0/packages/coding-agent/docs/extensions.md)、[extension types](https://github.com/earendil-works/pi/blob/v0.87.0/packages/coding-agent/src/core/extensions/types.ts) 与 [package rules](https://github.com/earendil-works/pi/blob/v0.87.0/packages/coding-agent/docs/packages.md) | Pi release `v0.87.0`, npm gitHead `16787ad5b2dc748047f314ca1bfe7708f30f54f3` | conceptual reference / runtime development type source | 当前 AKeel 使用的官方 `ExtensionAPI`、`ExtensionContext`、`context`/`before_agent_start`/session replacement contracts、`StringEnum` tool-schema guidance，以及核心包由 Pi 宿主提供、package 使用 `peerDependencies: "*"` 的发布合同；当前类型由根开发依赖精确锁定到 Pi `0.87.0`，AKeel 不复制 Pi 内部实现 | Pi 仓库 MIT；本仓库未复制其代码 |
...
| [Linux kernel pathname lookup documentation](https://docs.kernel.org/6.15/filesystems/path-lookup.html) 与 [Linux `path_resolution(7)`](https://man7.org/linux/man-pages/man7/path_resolution.7.html) | kernel documentation 6.15；Linux man-pages 6.15 语义基线 | conceptual reference | 只在 `core/compilation/path-evidence.ts` 采用路径组件查找、绝对/相对路径和 canonical 阶段解析时点的 Linux 语义；不宣称 fd 传递、OS sandbox 或 TOCTOU 消除 | Linux kernel documentation/man-pages 各自按上游许可证；本仓库未复制实现或正文 |
