...
| [astral-sh/uv](https://github.com/astral-sh/uv) release `0.12.6`；[uv project run documentation](https://docs.astral.sh/uv/concepts/projects/run/)；[uv license policy](https://docs.astral.sh/uv/reference/policies/license/) | conceptual reference / historical reference | `packages/access-gate/src/access-gate/access-decision/core/shell-words.ts` 中 `uv run`、版本/帮助和未建模子命令的分类参考；对应回归测试在 `tests/access-gate/access-decision/core/shell-policy.test.ts` | Apache-2.0 OR MIT（uv 官方 license policy）；固定参考 release `0.12.6`，仅作命令语义参考，AKeel 不分发 uv |
