...
已知且路径访问可完整证明、且不依赖仓库或用户配置执行 helper 的命令可进入普通 `inspect`/`modify`/`execute` 策略。Git 的 Inspect 选项契约继续有界解析展示标志（`--graph`、`--follow`、`--topo-order`、`--no-merges`、`--summary`、`--decorate`、`-p` 等）、带值过滤标量（`-S`、`-G`、`--grep`、`--author`、`--since`、`--until`、`--format`、`-L`、`--diff-filter` 等）、`diff --check`、`diff --find-renames`、`diff --find-copies`、`rev-parse --show-toplevel`/`--abbrev-ref`/`--symbolic-full-name` 及 `log`/`rev-list` 的纯数字行数限制缩写，并严格成对消费参数值。该参数边界只证明 argv 与路径事实，不证明 Git 运行期不读取 repository/user config 或执行 helper：`status`、`diff`、`log`、`show`、`rev-list` 与 `stash show` 保持 inspect 意图但同时标记 `opaque`，其中 `status` 因 Git 可选地刷新 index 而发行 `write` effect，显式全局 `--no-optional-locks` 只移除该 effect、不移除 opaque；它们由内置 `review`、`guided`、`develop` 分别 deny、ask、allow，且不能作为 bounded pipeline 的纯 inspect 上游。`branch --show-current`、受支持的 `rev-parse` 查询与 `stash list` 等不触及已识别 helper 面的 metadata-only 查询继续作为普通 inspect read。非 inspect 子命令不继承 Inspect 选项集；有界本地 `git add` 与带显式消息、无交互/外部驱动参数的 `git commit` 在 Git 控制面工件获得系统级绝对不可变写保护的前提下归类为 `modify`。显式 external driver（`--ext-diff`、`--textconv`）、无消息或交互式 commit、外部 transport/network helper、`git config` 及其他未形成受控合同的 helper-capable 操作继续固定进入 hard boundary。解释器脚本、`uv run`、`pytest`、包管理器委托、`npx` 及其他未建模运行期访问同样标记 opaque；opaque 风险由独立的 `commands.opaque` 策略轴与命令类别、路径 effects 共同求值，不因 allowed/blocked path 配置自动成为 sandbox。

路径选项和隐式 repository/project scope 必须进入 Canonical 统一解析；Admission 只消费已解析的路径候选，不重新理解程序参数。Git `-C`、`--git-dir` 和 `--work-tree` 已在 Canonical command-local cwd seam 中按 token 顺序解析，后续 repository、基本 path candidate、output 候选和显式项目内 `file://` remote 使用所得 cwd；完整 Git pathspec 语法、HTTPS/SSH 等外部 transport、hosted `file://`、alias、间接 config remote、`clone --separate-git-dir` 及其他尚未形成 Canonical seam 的 location 选项继续 fail-closed。Canonical 只把隐式 repository scope 绑定到 command cwd，不把运行期 Git config、gitfile/common-dir 或 helper 行为伪造成已证明路径；已识别但不可静态消除的 live inspection 风险进入 opaque policy，显式外部 helper 与 transport 继续 hard-deny。当前不增加 network policy 轴或执行期 Git sandbox。
...
Out of Scope: 网络独立授权、OS sandbox、Git hooks/npm lifecycle 的执行期拦截、完整 Git pathspec、远程/容器工具链方言、命令执行后的审计和旧配置兼容。当前不提供 `akeel_git_inspect` 或其他 live read-only Git broker：immutable Owner packet 已满足只读 child，待出现必须在创建后刷新 Git 且 Owner staleness refresh 被证明不足的受支持工作流，并具备可测试的无 Shell 执行、取消、输出预算与环境隔离接缝时再评估。`commands.opaque` 只表达用户对未证明执行风险的策略选择，不提供运行期沙箱或路径强制。
...
当只读 child 的任务依赖 Git 状态时，Owner packet 必须固定并携带适用的 status、staged/unstaged diff、untracked inventory、refs 与 base/head OID；该 packet 是 child 的 Git authority。只读 child 不重新执行 live Git、刷新 refs 或从共享 checkout 重建这些事实，缺失或疑似 stale 时只报告并由 Owner 重做 preflight、发行新 packet。具有写或可修改 Shell 能力的 child 继续位于独立 worktree，并仅在其 effective policy 允许时检查 live Git；worktree 隔离不把 Git config/helper 风险降级为纯读取。
