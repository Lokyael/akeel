...
## C-022: 会话级权限预设与运行时动态切换机制（Runtime Permission Switching）

> 本条只记录未来探索方向，不构成当前需求、路线图、架构采纳或实施承诺；当前不创建 Task、不修改代码，也不启动任何实现。

- **Why Not Now:** 当前单一全局 `policy.yaml` 配合静态开发策略已能维持正常编码，且 Greenfield 重构首要目标是建立并证明单次会话下的纯函数决策不变性；引入动态切换与会话状态管理涉及 Pi 宿主事件监听与终端 UI 交互，先作为会话治理候选立档。
- **Exploration Direction:** 探索保持 Policy Kernel 纯函数与深度不可变前提下的会话级策略切换机制：
  - **平铺具名策略集（Named Presets）**：在配置中支持多组独立的静态策略预设（如审查只读、安全交互、顺畅开发），彻底杜绝旧版 Profile 的多层继承与动态覆盖；
  - **会话外围状态切换**：通过宿主命令接口（如 `/policy`）按需重载当前会话绑定的策略快照，状态仅停留在运行时适配层，不反向渗透核心内核；
  - **终端人机可见性**：在终端界面提供轻量状态提示（如底栏预设指示器），保障人类对当前权限模式的知情权；
  - **提示词面零注入铁律（D-030/D-053）**：预设名称与策略内容永不注入模型上下文，权限受限时仅通过静态 Guidance 提示人类介入；
  - **子代理权限单向收紧**：派生子代理的权限上限受限于父会话，禁止子代理自行提权。
- **Trigger:** 用户在日常使用中高频出现“只读审查”与“自治编码”场景频繁切换的需求，或多代理协作中出现子代理无感降权的硬性诉求。
...
- **Exploration Direction:** 探索面向高危动作与复杂工程上下文的分层精细化治理架构：
  ...
  - **载体精细化（敏感凭据专道防护）**：联动既有受管面候选（C-016 环境变量单名契约、C-018 Worktree 物理隔离、C-021 内核级非特权沙盒），对真实敏感文件提供结构化单向保护而非简单的一刀切阻断。
