...
- Slice 9 PowerShell Canonical and production execution implemented: PowerShell v1 single-command literal tokenizer and syntax analyzer strictly reject pipelines, redirections, compounds, variables, and expressions fail-closed; admitted commands (Get-ChildItem, git.exe, node.exe, npm.cmd, unknown .exe) extract paths and effects; authorization kernel connects end-to-end to issue single-use execution tickets for verified executor execution.
