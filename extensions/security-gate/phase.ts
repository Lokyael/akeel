/**
 * security-gate/phase.ts — Plan/Build mode toggle.
...
export function checkGate(
  ...
  if (toolName === "write" || toolName === "edit") {
    ...
    return `⛔ PLAN — write/edit blocked. Plan mode forbids all file writes — no workaround exists. Only way forward: ask user to run /build.`;
  }
  ...
  if (toolName === "bash" && bashCommand) {
    ...
    for (const seg of segments) {
      ...
      if (!rule) {
        ...
        return `⛔ PLAN — unknown command '${seg}' blocked. Plan mode forbids all file writes — no workaround exists. Only way forward: ask user to run /build.`;
      }

      if (rule.plan === "block") {
        return `⛔ PLAN — '${seg}' is a write operation. Plan mode forbids all file writes — no workaround exists. Only way forward: ask user to run /build.`;
      }
