/**
 * rules.test.ts — Unit tests for wildcardMatch, buildRuleset, and evaluatePermission.
 *
 * Run: npx tsx extensions/security-gate/rules.test.ts
 */
...
function testConfig(overrides?: Partial<SecurityConfig>): SecurityConfig {
  return {
    level: "standard",
    sandbox: { enabled: false, filesystem: { denyRead: [], allowWrite: [], denyWrite: [], allowRead: [] }, network: { allowNetwork: true, allowedDomains: [] } },
    permission: {
      "*": "ask",
      path: { "*": "allow", ".env": "deny" },
      read: { "*": "allow" },
    ...
    audit: { enabled: false, logPath: "", redactSecrets: false },
    ...overrides,
  };
}
...
// Session rules appended last
{
  const sr: SessionRule[] = [{ surface: "write", pattern: "src/**", action: "allow", source: "session" }];
  const rules = buildRuleset("write", testConfig(), sr);
  const last = rules[rules.length - 1];
  assert(last.surface === "write" && last.action === "allow" && last.source === "session",
    "session rule appended last");
}
...
// Session rule overrides config rule (same surface + pattern)
{
  const rules = [
    { surface: "write", pattern: "*.lock", action: "deny" as const, source: "config" as const },
    { surface: "write", pattern: "*.lock", action: "allow" as const, source: "session" as const },
  ];
  const result = evaluatePermission("write", "yarn.lock", rules);
  assert(result.action === "allow", "session allow overrides config deny for same pattern");
}
